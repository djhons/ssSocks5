...
func Run(config Config, shadow func(net.Conn) net.Conn) {
	...
	for {
		...
		go createTunnel(config.Socks5Addr, c, shadow, func(c net.Conn) (socks.Addr, error) { return socks.Handshake(c) })
	}
}

func createTunnel(socksAddr string, c net.Conn, shadow func(net.Conn) net.Conn, getAddr func(net.Conn) (socks.Addr, error)) {
	...
	cratePort := *cratePort(socksAddr)
	go func() {
		for {
			if session.IsClosed() {
				fmt.Println("[-] 连接已断开", socksAddr)
				cratePort.Close()
				return
			}
		}
